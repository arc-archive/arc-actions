...
import { RequestDataExtractor } from '../src/runner/RequestDataExtractor.js';

describe('RequestDataExtractor()', () => {
  const data = new DataGenerator();

  function genHeaders(ct) {
    const cond = true;
    while(cond) {
      // this may returns empty string
      const result = HeadersGenerator.generateHeaders('request', ct);
      if (result) {
        return result;
      }
    }
  ...
  describe('extract()', () => {
    describe('request URL data', () => {
      let request;
      beforeEach(() => {
        request = data.generateHistoryObject();
      });
    ...
    describe('request headers data', () => {
      ...
      it('reads the header value', () => {
        const ct = HeadersGenerator.generateContentType();
    ...
    describe('response body', () => {
      describe('JSON body', () => {
        ...
        it('reads data in the JSON path', () => {
          response.headers = HeadersGenerator.generateHeaders('response', 'application/json');
      ...
      describe('XML body', () => {
        ...
        beforeEach(() => {
          ...
          // @ts-ignore
          executedRequest = { ...request, sourceMessage: '' };
          response = data.generateResponse({ noBody: true });
          response.headers = HeadersGenerator.generateHeaders('response', 'application/xml');
